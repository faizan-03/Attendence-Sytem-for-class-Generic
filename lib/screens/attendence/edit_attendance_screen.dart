...
class EditAttendanceScreen extends StatefulWidget {
  static const String routeName = '/edit-attendance';
...
class _EditAttendanceScreenState extends State<EditAttendanceScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (_course == null) {
      ...
      if (args != null) {
        ...
        _attendanceRecord = args['attendance'] as AttendanceRecord?;
  ...
  void _loadStudentsAndAttendance() {
    if (_course == null || _date == null || _attendanceRecord == null) return;
    ...
    _attendanceStatus = Map.from(_attendanceRecord!.studentStatus);
  ...
  Future<void> _updateAttendance() async {
    ...
    try {
      ...
      if (courseKey != null) {
        final existingKey = attendanceProvider.getAttendanceKey(
          _attendanceRecord!,
        );
        ...
        if (existingKey != null) {
          final updatedRecord = AttendanceRecord(
            ...
            classType: _attendanceRecord!.classType,
          );
          ...
          throw Exception('Could not find the original attendance record');
        }
      }
    } catch (error) {
      if (mounted) {
            ...
            onConfirm: () async {
              ...
              if (_attendanceRecord != null) {
                try {
                  ...
                  final attendanceKey = attendanceProvider.getAttendanceKey(
                    _attendanceRecord!,
                  );

                  if (attendanceKey != null) {
                    await attendanceProvider.deleteAttendance(attendanceKey);
                  ...
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Error deleting session: $error'),
                        backgroundColor: Colors.red,
                      ),
                    );
                  }
  ...
  Widget build(BuildContext context) {
    if (_course == null || _date == null || _attendanceRecord == null) {
      return Scaffold(
        ...
        body: const Center(child: Text('Invalid attendance record.')),
      );
    }
                    ...
                    Expanded(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 4),
                          Text(
                            'Class Type: ${_attendanceRecord!.effectiveClassType}',
                            style: TextStyle(
                              fontSize: 13,
